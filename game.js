...
var endGame = function () {
    ...
    for (var row = 0; row < GAME.rows; row++) {
        for (var col = 0; col < GAME.cols; col++) {
            ...
            if(cell.getAttribute("marked") && GAME.layout[row][col]!=-1){
...
var toggleMarkCell = function (row, col, marked) {
    ...
    var cell = GAME.Cells[row][col];
    if(cell.classList.getAttribute("marked") != true){
        cell.classList.setAttribute("marked","true");
    }
    if (validMove(row, col)) {
        ...
        if (marked) {
            ...
            cell.classList.setAttribute("marked","false");
            cell.removeChild(cell.firstChild);
            GAME.flaggedCells--;
        } else {
...
var resetNeighbors = function (row, col) {
    ...
    for (i = row - 1; i <= row + 1; i++) {
        for (j = col - 1; j <= col + 1; j++) {
            ...
            // https://www.w3schools.com/jsref/prop_element_classlist.asp
            var cell = GAME.Cells[i][j];
            if(i>=0 && i<GAME.rows && i!=row && j>=0 && j<GAME.cols && j!=col && cell.classList.contains("closed")){
...
var updateNeighborCounts = function (row, col) {
    ...
    for(i=row-1;i>=0 && i<GAME.rows && i<=row+1;i++){
        for(j=col-1;j>=0 && j<GAME.cols && j<=col+1;j++){
...
var getFormattedTime = function(timeInSeconds) {
    ...
    var hours   = format(Math.floor(sec_num / 3600),2);
    var minutes = format(Math.floor((sec_num - (hours * 3600)) / 60),2);
    var seconds = format(sec_num - (hours * 3600) - (minutes * 60),2);
